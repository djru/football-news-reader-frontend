...
class App extends Component {
  constructor(props) {
    ...
    this.idleDebouncer = _.debounce(() => {
      this.props.dispatch({ type: ACTIONS.SET_IDLE, data: true })
    }, minute)
  }
  componentDidMount() {
  ...
  shouldLoadMore() {
    return (
      !this.props.scrollState.loading &&
      !this.props.scrollState.end &&
  ...
  fetchNextPageOfTweets(pageToLoad) {
    ...
    axios
      .get('https://api.fbnews.ml/tweets?size=3&page=' + pageToLoad)
      .then(res => {
        if (res.data.length) {
          this.props.dispatch({
            type: ACTIONS.PUSH_NEW_TWEETS,
            data: res.data,
          })
  ...
  render() {
    return (
      <div
        className={
          'container' +
          (this.props.settings.darkMode === true ? ' dark-mode' : '')
        }
      >
        <div className="dark-mode-block">
          <div>
            ...
            <input
              ...
              onChange={e => {
                this.updateSettings('darkMode', e.target.checked)
              }}
            />
          </div>
          <div>
            ...
            <input
              type="checkbox"
              checked={this.props.settings.autoRefresh}
              onChange={e => {
        ...
        <div className="header-block">
          <h1>Football News Reader</h1>
          <h2>Tweets About Football</h2>
        </div>
        <div className="tweet-container">
          {this.props.tweetsData.tweets.map(t => (
            <TweetEmbed
              tId={t.tweet_id}
              key={t._id + this.props.settings.darkMode}
            />
          ))}
        </div>
